...
\begin{itemize}
    \item get\_frontal\_face\_detector() - funkcja nie przyjmuje żadnych parametrów, zwraca wytrenowany obiekt umożliwiający detekcję twarzy. Model został wyszkolony przy pomocy algorytmu działającego w oparciu o klasyfikator SVM i deskryptor HOG, opisany w podrozdziale \ref{sec:svmhog}. 
...
\begin{itemize}
    \item getAffineTransform() - jako dane wejściowe należy podać dwa zbiory punktów, dla których obliczana jest macierz transformacji afimicznej.
    \item warpAffine() - funkcja dokonuje przekształcenia obrazu za pomocą macierzy rotacji będącej wynikiem wcześniejszej operacji.
\end{itemize}
...
Jako wynik otrzymamy zmodyfikowany obraz, na rysunku \ref{triangle_result} zwizualizowano ten krok dla wybranej iteracji. Po otrzymaniu nowej figury, należy dodać ją do obszaru twarzy, który docelowo zastąpi aktualny rejon oblicza awatara. Funkcja add\_triangle\_to\_new\_rect\_area() odpowiada za to zadanie. W celu usunięcia ewentualnych szumów etap ten uprzedzono zastosowaniem binaryzacji progowej.
...
Schemat opisany w tej sekcji należy powtórzyć dla każdego trójkąta zwróconego przez funkcję delaunay\_triangulation(). W skończonej pętli następuje modyfikacja wszystkich figur i jednoczesna rekonstrukcja danych fragmentów nowego obszaru twarzy awatara. Finalny efekt można obserwować na obrazie \ref{mask_before_avatar} przedstawionym w następnej sekcji. Trójkąty były dopasowane do odpowiadających im figur z obrazu źródłowego, także efektem jest zmodyfikowany obszar twarzy o takich samych wymiarach jak twarz źródłowa (\ref{src_img}).
...
\begin{itemize}
    \item PiecewiseAffineTransform() - inicjalizacja pustego obiektu, który umożliwia transformację bazując na dwóch równoliczncych zestawach punktów. Jej działanie oparte jest na wykorzystaniu triangulacji Delone.
    \item estimate() - funkcja wywołana na otrzymanym wcześniej obiekcie przyjmuje dwa zestawy punktów, dla których ma nastapiąć przemapowanie. W przypadku działania owego programu wybrano elementy oznaczone na Rys. \ref{fig:specificLandmarks} kolorem czerwonym. Są to wybrane punkty charakterystyczne, które z reguły w przypadku podstawowych zmian mimiki pozostają niezmienne - linia szczęki, główny zarys nosa oraz kąciki oczu. Wybór akurat tych punktów zapewni zachowanie odpowiednich proporcji twarzy w trakcie transformacji obszaru.
    \item warp() - funkcja dokonująca transformacji płaszczyzny na podstawie danych wejściowych. 
\end{itemize}
...
Aby podmienienić oblicze awatara na zmodyfikowany obszar wystarczy skorzystać z podstawowej operacji przetwarzania obrazów, czyli funkcji bitwise\_and() udostępnionej przez bibliotekę OpenCV. Umożliwia ona zastosowanie na zdjęciu źródłowym maski o wymiarach nowego obszaru twarzy, czego rezultatem jest obraz przedstawiony na Rys. \ref{without_face_avatar}.
...
Grafika przedstawiona poniżej przedstawia strukturę aplikacji webowej. Zawarte na niej funkcje, zostały opisane we wcześniejszych sekcjach oraz pozostałe moduły zaimplementowane na potrzeby stworzenia poprawnie działającej aplikacji.
